...
class ThemeModeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (iconData, action, actionLabel) = switch (brightness) {
      Brightness.light => (Icons.dark_mode_outlined, AdaptiveTheme.of(context).setDark, 'Switch to dark'),
      Brightness.dark => (Icons.light_mode_outlined, AdaptiveTheme.of(context).setLight, 'Switch to light'),
    };
    ...
    return switch (variant) {
      ...
      _Variant.outlined => OutlinedButton.icon(
        ...
        style: OutlinedButton.styleFrom(side: BorderSide(color: colorScheme.primary.withValues(alpha: 0.5))),
      ),
    };
  }
}
